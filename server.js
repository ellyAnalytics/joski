      ...
      (err, row) => {
        if (err) {
          ...
          db.run(
            ...
            (err2) => {
              if (err2)
                console.error("Failed to insert cashier:", err2.message);
              else
                console.log(
                  "✅ Default cashier created: username='cashier', password='cash123'"
                );
            }
          );
        }
...
function requireCashierOrAdmin(req, res, next) {
  if (
    req.session &&
    (req.session.role === "admin" || req.session.role === "cashier")
  )
    return next();

  res.render("forbidden", {
    session: { error: "❌ Forbidden: You don't have access to this page" },
  });
}
                  ...
                  db.get(
                    ...
                    (err, monthRow) => {
                      ...
                      db.get(
                        ...
                        [currentMonth],
                        (err, monthProfitRow) => {
                          const monthsProfit = monthProfitRow
                            ? monthProfitRow.profit
                            : 0;
  ...
  try {
    ...
    db.serialize(() => {
      cart.forEach((item) => {
        ...
        //update products
        db.get(
          "SELECT remainingQty FROM products WHERE code = ?",
          [item.productCode],
          (err, row) => {
            if (err || !row) {
          ...
          db.run(
           ...
           WHERE code = ?`,
            ...
            function (err3) {
              if (err3)
                console.error(
                  "Failed to update stock after credit:",
                  err3.message
                );
...
app.post("/credit/pay/:id", (req, res) => {
  ...
  db.get("SELECT * FROM credits WHERE id = ?", [creditId], (err, credit) => {
    ...
    if (newPaid < credit.total) {
      ...
      db.run(
        "UPDATE credits SET paid = ? WHERE id = ?",
        [newPaid, creditId],
        (err2) => {
